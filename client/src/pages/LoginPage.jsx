  ...
  return (
    ...
    <form onSubmit={onSubmitHandler} autoComplete="off" className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
      ...
      {!isDataSubmitted && (
        ...
        type="email" autoComplete="new-email" placeholder='Email Address' required 
        className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500'/>
        <div className='relative'>
        <input onChange={(e)=>setPassword(e.target.value)} value={password}
        type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder='Password' required 
        className='w-full p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500'/>
        <button type='button' onClick={()=>setShowPassword(!showPassword)} className='absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-white'>
          {showPassword ? "👁️" : "👁️‍🗨️"}
        </button>
        </div>
        </>
      ...
      <div className='flex flex-col gap-2'>
        {currState === "Sign up" ? (
          ...
          <span onClick={()=>{setCurrState("Login"); setIsDataSubmitted(false); setEmail(""); setPassword("")}}
          className='font-medium text-violet-500 cursor-pointer'> Login here</span>
          </p>
        ) : (
          <p className='text-sm text-gray-600'>Create an account  
          <span onClick={()=>{setCurrState("Sign up"); setEmail(""); setPassword(""); setFullName("")}}
